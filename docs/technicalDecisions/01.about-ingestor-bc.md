...
5. Run `bin/console messenger:consume redis-async` to process the Redis messages: this action will fetch the messages from Redis, persist the aggregate roots in the database and publish the Domain events.
6. Check the rabbitMQ UI at http://localhost:15672/ (username: admin, password: admin) to see the published messages in the `domain_events` exchange.
7. You can also check the database (tables `events` and `event_zones`) to see the persisted data. Use your preferred MySQL client (host: localhost:3306, username: root, password: root, database: ingestor_db).
8. Run `bin/console messenger:consume rabbitmq-async` to see how the Catalog BC reacts to the Domain events and start updating its own projections.

<img src="img/step3.png" width="200"/> <img src="img/step4.png" width="200"/> <img src="img/step6.png" width="200"/>
